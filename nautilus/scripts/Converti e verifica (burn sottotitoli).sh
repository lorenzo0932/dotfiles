...
do
    ...
    output="/home/lorenzo/Video/Convertiti/$(basename "$i" | cut -f 1 -d '.').mp4"
    log_file="${output}.log"

    # Conversione del file con ffmpeg
    nice -n 5 ffmpeg -i "$i" -vf "subtitles='$i'" -c:v libx265 -crf 23 -preset veryfast -threads 12 -x265-params hist-scenecut=1 -sn -c:a copy "$output"

    # Verifica del file convertito; gli errori vengono reindirizzati nel file .log
    nice -n 5 ffmpeg -v error -i "$output" -f null - 2>"$log_file"

    # Se il file log è vuoto (0 byte), lo elimina e cancella il file originale, altrimenti aggiunge il nome del file alla lista degli errori
    if [ ! -s "$output_verifica" ]; then
        rm "$log_file"
        rm "$i"
    else
        fail_list="${fail_list}\n$(basename "$i")"
