...
for i in $NAUTILUS_SCRIPT_SELECTED_FILE_PATHS; do
    ...
    while [ $attempt -le $MAX_RETRIES ]; do
        ...
        ffmpeg_command=( ffmpeg -y -i "$original_path" -map 0 -c:v libx265 -crf 23 -preset veryfast -threads 12 -x265-params hist-scenecut=1 -c:a copy -c:s copy "$intermediate_output" )
