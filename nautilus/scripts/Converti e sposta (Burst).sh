#!/bin/bash
# AniEngine Burst - (v18 + C++ Adaptive Logic)
START_TIME=$(date +%s)
export IFS=$'\n'
SESSION_ID="$$"
...
if [ "$TOTAL_FILES" -gt 1 ]; then
    MAX_CONCURRENT=2
    NUM_CHUNKS=3     # 2 video * 4 chunk = 8 processi (come da immagine)
else
    MAX_CONCURRENT=1
    NUM_CHUNKS=4     # 1 video * 4 chunk = 4 processi
fi
...
process_file() {
    ...
    # Gestione RAM/Disk dinamica
    file_size_kb=$(du -k "$i" | cut -f1)
    ram_free_kb=$(df -k /dev/shm | awk 'NR==2 {print $4}')
    WORK_ROOT=$([ "$file_size_kb" -lt $((ram_free_kb * 70 / 100)) ] && echo "/dev/shm/burst_job_${RANDOM}_$SESSION_ID" || echo "/var/tmp/burst_job_${RANDOM}_$SESSION_ID")
    mkdir -p "$WORK_ROOT"
    ...
    (
        ...
        for part in "$WORK_ROOT"/s[0-9]*.mp4; do
            ffmpeg -y -i "$part" -c:v libx265 -crf 23 -preset veryfast -threads $THREADS_PER_CHUNK -x265-params "hist-scenecut=1" -c:a copy "${part%.*}.enc.mp4" >/dev/null 2>&1 &
            pids+=($!)
        done
