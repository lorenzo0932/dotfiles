...
do
    ...
    output="/home/lorenzo/Video/Convertiti/$(basename "$i" | cut -f 1 -d '.').mkv"
    log_file="${output}.log"

    # Conversione del file con ffmpeg
    nice -n 5 ffmpeg -i "$i" -map 0 -c:v libx265 -crf 23 -preset veryfast -threads 12 -x265-params hist-scenecut=1 -c:a copy -c:s copy "$output"
