...
export default class FullscreenCommandPreferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        this._addFilePicker(window, startRow);
        this._addFilePicker(window, stopRow);
    ...
    _addFilePicker(window, row) {
        const button = new Gtk.Button({
            icon_name: 'folder-symbolic',
            valign: Gtk.Align.CENTER,
            tooltip_text: 'Choose a script…',
            css_classes: ['flat'],
        });
        button.connect('clicked', () => {
            const dialog = new Gtk.FileDialog({title: 'Choose script'});
            dialog.open(window, null, (dialog_, res) => {
                let file;
                try {
                    file = dialog_.open_finish(res);
                } catch (e) {
                    return;
                }
                row.text = GLib.shell_quote(file.get_path());
            });
        });
